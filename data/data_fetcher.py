...
class DataFetcher:
    ...
    @staticmethod
    def fetch_stocks(symbols: List[str], start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    @staticmethod
    def fetch_crypto(symbols: List[str], start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    @staticmethod
    def fetch_commodities(symbols: List[str], start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    @staticmethod
    def fetch_forex(symbols: List[str], start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """ Fetch forex data """
    ...
    def fetch_all_market_data(self, start_date: datetime, end_date: datetime) -> Dict[str, pd.DataFrame]:
        """ Fetch data for all asset classes. Returns a dictionary with keys: 'stocks', 'crypto', 'commodities', 'forex' """
        all_data = dict()

        try:
            all_data['stocks'] = self.fetch_stocks(self.config.ASSETS['stocks'], start_date, end_date)
        except Exception as e:
            logger.error(f"Error fetching stocks: {e}")
            all_data['stocks'] = pd.DataFrame()

        try:
            all_data['crypto'] = self.fetch_crypto(self.config.ASSETS['crypto'], start_date, end_date)
        except Exception as e:
            logger.error(f"Error fetching crypto: {e}")
            all_data['crypto'] = pd.DataFrame()

        try:
            all_data['commodities'] = self.fetch_commodities(self.config.ASSETS['commodities'], start_date, end_date)
        except Exception as e:
            logger.error(f"Error fetching commodities: {e}")
            all_data['commodities'] = pd.DataFrame()

        try:
            all_data['forex'] = self.fetch_forex(self.config.ASSETS['forex'], start_date, end_date)
        except Exception as e:
